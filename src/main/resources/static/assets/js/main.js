...
(function () {
  ...
  if (typeof i18next !== 'undefined' && typeof i18NextHttpBackend !== 'undefined') {
    ...
    const initialLang = savedLang || (window.templateCustomizer ? window.templateCustomizer.settings.lang : 'en');
    ...
    i18next
      ...
      .init({
        ...
        detection: {
          ...
          lookupLocalStorage: 'templateCustomizer-' + templateName + '--Lang',
          caches: ['localStorage']
        }
      })
      .then(function (t) {
    ...
    for (let i = 0; i < dropdownItems.length; i++) {
      dropdownItems[i].addEventListener('click', function () {
        ...
        i18next.changeLanguage(currentLanguage, (err, t) => {
          ...
          // Сохраняем язык в localStorage напрямую, не используя templateCustomizer.setLang()
          // так как он не поддерживает ru и uk
  ...
  function localize() {
    ...
    // Обновляем активный пункт в dropdown БЕЗ клика (чтобы избежать бесконечного цикла)
    ...
    i18nList.forEach(function (item) {
      const translatedText = i18next.t(item.dataset.i18n);
      // Для элемента title нужно обновлять document.title
      if (item.tagName.toLowerCase() === 'title') {
